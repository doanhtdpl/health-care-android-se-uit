...
public class HeartRateDAO extends Database {
	...
	public boolean insertHeartRate(HeartRateDTO heartRate) {
		try {
			...
			db.insert(COLUMN_HEARTRATE_ID, null, contentValues);
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	public Integer deleteHeartRate(Integer id) {
		SQLiteDatabase db = this.getWritableDatabase();
		return db.delete(HEARTRATE_TABLE, COLUMN_HEARTRATE_ID + " = ? ",
	...
	public ArrayList<HeartRateDTO> getListHeartRate(Integer userId) {
		...
		Cursor res = db.rawQuery("select * from " + HEARTRATE_TABLE + " where "
				+ COLUMN_HEARTRATE_USER_ID + " = ",
	...
	public Integer getNewHeartRateaId() {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor res = db.rawQuery("select " + COLUMN_HEARTRATE_ID + " from "
